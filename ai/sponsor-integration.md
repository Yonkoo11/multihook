...
| Birdeye (Eitherway angle) | n/a | DROPPED | `submissions/eitherway-birdeye-DROPPED.md` — mainnet only; replaced by QuickNode angle | DROPPED |
| QuickNode (Eitherway $20K) | 3 | 3 | `app/src/programs.ts` multi-provider RPC priority chain; QuickNode now position 2 (RPC Fast position 1); footer surfaces active provider; `submissions/eitherway-quicknode.md` | 0 — depth target met |
| RPC Fast ($10K) | 3 | 3 | `app/src/programs.ts` priority position 1; `app/src/main.ts` footer label; `submissions/rpcfast.md` | 0 — depth target met |
| Umbra ($10K) | 3 | 3 | `app/src/umbra-shield.ts` shieldViaUmbra wraps `getPublicBalanceToEncryptedBalanceDirectDepositorFunction` from `@umbra-privacy/sdk`; Step 05 station in `app/index.html`; dynamic import keeps the 2 MB SDK chunk lazy; `submissions/umbra-privacy.md` documents the composition story (compliance fires BEFORE privacy) | 0 — depth target met (UNTESTED end-to-end on devnet — confidence MEDIUM until live shield run) |
| Helius audit feed (depth-4 push) | 4 | 4 | `app/src/audit-feed.ts` — getSignaturesForAddress + getTransaction + decodeAuditEvent pipeline; `app/src/main.ts` renders the feed below the receipt with status pills + reject reasons | 0 — depth target met |
| Phantom SIWS (depth-4 push) | 4 | 4 | `app/src/siws.ts` — wallet-standard solana:signIn flow; `app/src/wallet.ts:signIn` adapter; topbar pill in `app/src/main.ts:renderWallet`; localStorage session persistence | 0 — depth target met (UNTESTED in real Phantom on the new build — confidence MEDIUM) |
